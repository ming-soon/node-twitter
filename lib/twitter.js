...
function Twitter(options) {
  ...
  this.options = extend({
    ...
    oauth_base: 'https://api.twitter.com/oauth2/token',
    request_options: {
      headers: {
        Accept: '*/*',
        Connection: 'close',
 ...
 */
Twitter.prototype.obtainToken = function(callback) {
  var self = this;
  var promise = false;
  if (callback === undefined && this.allow_promise) {
    promise = true;
  }

  var keyEncoded = encodeURIComponent(this.options.consumer_key);
  var secretEncoded = encodeURIComponent(this.options.consumer_secret);
  var credentialsEncoded = Buffer.from(keyEncoded + ':' + secretEncoded).toString('base64');

  var options = {
    method: 'POST',
    url: this.options.oauth_base,
    headers: {
